...
bool lastRunningState; // Stores last running state to allow turning the stepper
                       // off after moves.
...
void loop() {
  ...
  if (sensorTesting) {
    ...
    if (getLimitState() == LIMIT_SENSOR_ACTIVE_STATE && !calibrating &&
        stepper.isRunning() && stepper.targetPosition() < 0) {
    ...
    if (getHomeState() == HOME_SENSOR_ACTIVE_STATE && homed && !calibrating &&
        stepper.isRunning() && stepper.distanceToGo() > 0) {
